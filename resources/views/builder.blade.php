...
<html lang="en">
  <head>
    ...
    <base href="{{ url('', []) }}">
    <title>Domain Store | Web builder</title>
    <base href="{{ url('assets', []) }}" target="_blank">
    <link href="{{ url('css/editor.css', []) }}" rel="stylesheet">
    <link href="{{ url('css/line-awesome.css', []) }}" rel="stylesheet">
  </head>
<body>
	...
	<div id="vvveb-builder">
				...
				<div id="top-panel">
				<h3 class="float-left"><a href="{{url('/dashboard')}}" class="text-decoration-none">Domain Store</a></h3>					
...
<!-- builder code-->
<script src="{{ url('/libs/builder/builder.js', []) }}"></script>	
<!-- undo manager-->
<script src="{{ url('/libs/builder/undo.js', []) }}"></script>	
<!-- inputs-->
<script src="{{ url('/libs/builder/inputs.js', []) }}"></script>	
...
<!-- components-->
{{-- <script src="{{ url('/libs/builder/components-server.js', []) }}"></script>	 --}}
<script src="{{ url('/libs/builder/components-bootstrap4.js', []) }}"></script>	
<script src="{{ url('/libs/builder/components-widgets.js', []) }}"></script>	

<!-- blocks-->
<script src="{{ url('/libs/builder/blocks-bootstrap4.js', []) }}"></script>	

<!-- plugins -->

<!-- code mirror - code editor syntax highlight -->
<link href="{{ url('/libs/codemirror/theme/material.css', []) }}" rel="stylesheet"/>
<link href="{{ url('/libs/codemirror/lib/codemirror.css', []) }}" rel="stylesheet"/>
<script src="{{ url('/libs/codemirror/lib/codemirror.js', []) }}"></script>
<script src="{{ url('/libs/codemirror/lib/xml.js', []) }}"></script>
<script src="{{ url('/libs/codemirror/lib/formatting.js', []) }}"></script>
<script src="{{ url('/libs/builder/plugin-codemirror.js', []) }}"></script>	

<!-- jszip - download page as zip -->
<!-- script src="libs/jszip/jszip.min.js"></script>-->
<script src="{{ url('libs/builder/plugin-jszip.js', []) }}"></script>


<!-- autocomplete plugin used by autocomplete input-->
{{-- <script src="{{ url('libs/autocomplete/jquery.autocomplete.js', []) }}"></script>	 --}}
<?php
	$directory 	=   "temp/".Auth::user()->id;
	$url 		=   $data ? $data->url : '';
	$parse 		=	parse_url($url);
	$filename 	=	!empty($parse['host']) ? $parse['host'] : ''; 
...
if (!file_exists($directory)) {
	...
	if (!file_exists("$directory/$filename")) {
		$myfile = fopen("$directory/$filename.html", "w") or die("Unable to open file!");
		$txt = "<!DOCTYPE html><html lang='en'><head></head><body><h2 class='mt-4 text-center'>$filename</h2> <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Perspiciatis, corrupti maxime iusto inventore veritatis sit blanditiis odit nihil suscipit? Neque mollitia provident doloribus reiciendis voluptatum esse deleniti at est adipisci.</p></body></html>";
		fwrite($myfile, $txt);
		fclose($myfile);
	}
} else {
...
$(document).ready(function() 
{
	...
	Vvveb.Builder.init("<?php echo $path?>.html", function() {
	...
	[		
		...
		?>
		{name:"<?php echo ucfirst($pathInfo['filename']);?>", title:"<?php echo ucfirst($pathInfo['filename']);?>",folder:"My Templates",  url: "<?php echo $pathInfo['dirname'].'/'.$pathInfo['basename'];?>"},
		<?php } ?>
		// {name:"narrow-jumbotron", title:"Jumbotron",  url: "demo/narrow-jumbotron/index.html", assets: ['demo/narrow-jumbotron/narrow-jumbotron.css','temp/asset/main.js']},
		{name:"album", title:"Album",  url: "demo/album/index.html", folder:"content", assets: ['temp/asset/main.js']},
		{name:"blog", title:"Blog",  url: "demo/blog/index.html", folder:"content", assets: ['demo/blog/blog.csss','temp/asset/main.js']},
	...
	Vvveb.FileManager.loadPage("<?php echo ucfirst($filename)?>");
	// Vvveb.FileManager.loadPage("album");
